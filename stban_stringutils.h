...
stban_sv stban_sv_from_substr(char *data, size_t from, size_t to) {
  stban_sv sv;
  sv.data = (data + from);
  assert((to >= from) && "from needs to be greater than too");
  sv.len = to - from;
  return sv;
}
...
stban_sv stban_sv_chop(stban_sv *sv, size_t n) {
  assert((n < sv[0].len && n >= 0) && "Index out of range");
...
stban_sb stban_sb_create(){
  ...
  assert((sb.data != NULL) &&
	 "malloc failed for string builder :: stban_sb_create");
  sb.len = 0;
  return sb;  
}
...
stban_sb stban_sb_from_substr(char *str, size_t from, size_t to){
  assert((to >= from)
	 && "range invalid, to must be greater than or equal to from :: stban_sb_from_substr");
  ...
  sb.data = (char *) malloc(sb.capacity * sizeof(char));
  assert((sb.data != NULL) &&
	 "malloc failed for string builder :: stban_sb_from_substr");
...
void stban_sb_append_substr(stban_sb *sb, char *str, size_t from, size_t to) {
  assert((to >= from)
	 && "range invalid, to must be greater than or equal to from :: stban_sb_append_substr");
  ...
  if(capacity != sb[0].capacity){
    ...
    assert((sb[0].data != NULL) &&
	   "realloc failed for string builder :: stban_sb_append_substr");
  }
...
void stban_sb_append_null(stban_sb *sb){
  if(sb[0].len >= sb[0].capacity){
    ...
    sb[0].data = (char *) realloc(sb[0].data, sb[0].capacity * sizeof(char));
    assert((sb[0].data != NULL) &&
	   "realloc failed for string builder :: stban_sb_append_null");
  }
  sb[0].data[sb[0].len] = '\0';
}

void stban_sb_chop(stban_sb *left, stban_sb *right, size_t n) {
  assert((n >= 0 && n < right[0].len)
	 && "index out of range :: stban_sb_chop");
  stban_sb_append_substr(left, right[0].data, 0, n);
  right[0].data = (char *) memmove(right[0].data, right[0].data + n + 1, right[0].len - n - 1);
  assert((right[0].data != NULL) &&
	 "memmove failed :: stban_sb_chop");
